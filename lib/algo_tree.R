...
regtreealg <- function(df,LHS,RHS,from,to,
                       parameter=list(minsplit=10,maxdepth=30,mingain=0,numericfactors=F),quiet=T) {
  if(!quiet) {
  ...
  thistree <- rpart::rpart(formula=modelformula,data=factorsNumeric(df[from,]),method="anova",
            control=rpart::rpart.control(minsplit=parameter$minsplit,
...
#' @export
regtree <- function(minsplits=c(1,5,10,20,30,40,50,70,100,150,200,500),maxdepths=30,mingains=0,numericfactors=F) {
  paramgrid <- expand.grid(minsplit=minsplits,maxdepth=maxdepths,mingain=mingains,numericfactors=numericfactors)
